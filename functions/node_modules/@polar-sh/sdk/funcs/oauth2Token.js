 ...
 */
async function oauth2Token(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.Oauth2RequestTokenRequestBody$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(200, components.TokenResponse$inboundSchema), M.fail(["4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
