 ...
 */
async function oauth2Revoke(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.Oauth2RevokeTokenRevokeTokenRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(200, components.RevokeTokenResponse$inboundSchema), M.fail(["4XX", "5XX"]))(response);
    if (!result.ok) {
        return result;
    }
    return result;
