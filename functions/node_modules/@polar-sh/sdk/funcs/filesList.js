...
async function filesList(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.FilesListRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, operations.FilesListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, errors.HTTPValidationError$inboundSchema), M.fail(["4XX", "5XX"]))(response, { extraFields: responseFields });
    if (!result.ok) {
