...
async function ordersList(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.OrdersListRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, operations.OrdersListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, errors.HTTPValidationError$inboundSchema), M.fail(["4XX", "5XX"]))(response, { extraFields: responseFields });
    if (!result.ok) {
