...
async function organizationsCreate(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => components.OrganizationCreate$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(201, components.Organization$inboundSchema), M.jsonErr(422, errors.HTTPValidationError$inboundSchema), M.fail(["4XX", "5XX"]))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
